...
<script>
    ...
    async function fetchPets(filter) {
        try {
            const response = await fetch('filter-surrender.php', {
                method: 'POST',
                headers: {
...
<script>
    ...
    document.getElementById('card-container').addEventListener('click', function (e) {
        if (e.target && e.target.classList.contains('approval-btn')) {
            ...
            const surrenderId = e.target.getAttribute('data-surrender-id');
            const approve = 1;
            window.location.href = `send-email.php?pet_id=${petId}&requester_id=${requesterId}&surrender_id=${surrenderId}&approve=${approve}`;
        }
    });
